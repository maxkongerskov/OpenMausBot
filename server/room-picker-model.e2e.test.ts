...
// override must not change the engine a group turn actually runs. Two fake
// engines with separate dump files prove which instance ran.
...
describe("room turns follow the bot default, not a thread-only picker", () => {
  it("keeps a 1:1 thread override off the group engine", async () => {
    ...
    await expect.poll(() => existsSync(defaultDump), { timeout: 20_000 }).toBe(true);
    expect(existsSync(pickerDump)).toBe(false);
    const dump = JSON.parse(readFileSync(defaultDump, "utf8")) as { systemPrompt?: string };
